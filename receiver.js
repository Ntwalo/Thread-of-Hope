...
import { getFirestore, doc, setDoc, collection, serverTimestamp, query, where, getDocs } from "https://www.gstatic.com/firebasejs/10.11.1/firebase-firestore.js";
...
document.getElementById('receiverForm').addEventListener('submit', async (e) => {
  ...
  console.log(`Form Values: Name=${name}, Phone=${phone}, Cloth Type=${cloth_type}, size=${size}`);
  ...
  onAuthStateChanged(auth, async (user) => {
    ...
    if (user) {
      ...
      try {
        // Query donations collection for available clothes across all users
        const donationsRef = collection(db, "donations");
        const q = query(donationsRef, where("cloth_type", "==", cloth_type), where("size", "==", size));
        console.log('Query created'); // Debugging log

        const querySnapshot = await getDocs(q);
        console.log('Query executed'); // Debugging log

        let availableClothes = [];
        querySnapshot.forEach((doc) => {
          console.log(`Document found: ${JSON.stringify(doc.data())}`); // Log each document
        ...
        if (availableClothes.length > 0) {
          ...
          if (confirm("Clothes are available! Would you like to receive them?")) {
            ...
            // Save request data to Firestore under the receivers collection
            await setDoc(doc(collection(db, `receivers`), requestId), requestData);
